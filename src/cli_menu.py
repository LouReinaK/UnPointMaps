...
class CLIMenu:
    ...
    def _ask_exclude_big_events(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, event in enumerate(self.events[:5], 1):
             print(f"  #{i}: {event['label']} ({event['start_date_str']} - {event['end_date_str']}) - {event['total_entries']} entries")
        
        choice = input("\nDo you want to exclude major events to avoid skewing? [y/N]: ").strip().lower()
        if choice == 'y':
            print("Select which events to exclude (comma separated numbers, e.g. '1, 2'), or 'all' for top 5:")
    ...
    def _handle_isolate_event(self, df: pd.DataFrame) -> Optional[pd.DataFrame]:
        self._ensure_events_loaded(df)
        
        print("\n--- Top Detected Events ---")
        for i, event in enumerate(self.events[:10], 1):
            print(f"#{i}: {event['label']}")
            print(f"    {event['start_date_str']} - {event['end_date_str']} ({event['total_entries']} entries)")
    ...
    def _handle_year_filter(self, df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df_filtered = self.time_filter.filter_by_time_range(df, start_year, end_year)
    ...
    def _handle_hour_filter(self, df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df_filtered = self.time_filter.filter_by_hour(df, start_hour, end_hour)
            print(f"Filtered to {len(df_filtered)} entries.")
