...
def main():
    ...
    try:
        ...
        cols = [' date_taken_year', ' date_taken_month', ' date_taken_day', ' date_taken_hour', ' id', ' tags', ' title']
        df = pd.read_csv(file_path, usecols=cols, low_memory=False)
    except ValueError:
        print("Specific columns not found, loading header to inspect...")
        df_header = pd.read_csv(file_path, nrows=0)
        # Try to find columns gracefully
        usecols = []
        for col in df_header.columns:
            clean_col = col.strip()
            if 'date_taken' in clean_col or clean_col in ['id', 'tags', 'title']:
                usecols.append(col)
    ...
    # Process hour separately since it's not needed for the date object itself but for the analysis
    if 'date_taken_hour' in df.columns:
    ...
    print(f"Dropped {dropped_count} rows with invalid dates. Remaining: {len(df)}")
    ...
    df = df[df['date'] <= now]
    dropped_count = initial_count - len(df)
    print(f"Dropped {dropped_count} rows with future dates. Remaining: {len(df)}")

    # Filter dates before 2000
    year_2000 = pd.Timestamp('2000-01-01')
    initial_count = len(df)
    df = df[df['date'] >= year_2000]
    dropped_count = initial_count - len(df)
    print(f"Dropped {dropped_count} rows before year 2000. Remaining: {len(df)}")
    ...
    ax3.hist(df['day_of_year'], bins=366, range=(1, 366), edgecolor='none', alpha=0.7)
    ...
    ax4.hist(df['week_of_year'], bins=53, range=(1, 53), edgecolor='black', alpha=0.7)
...
def detect_events_by_entries(df):
    ...
    print(f"Threshold for event days: {threshold:.2f} entries (Mean: {daily_counts.mean():.2f}, Std: {daily_counts.std():.2f})")
    ...
    sorted_events = sorted(event_stats, key=lambda x: x['total_entries'], reverse=True)
    ...
    for i, event in enumerate(sorted_events[:10], 1):
        ...
        if labeler is not None:
            try:
                ...
                # Note: this ignores year, as requested (aggregated recurring events)
                ...
                print(f"\r    Label: {result.label} (Conf: {result.confidence:.2f})")
