...
def get_alpha_shape(points, alpha=None, auto_alpha_quantile=0.95):
    ...
    try:
        ...
        circum_r[valid_tri_mask] = (
            a[valid_tri_mask] * b[valid_tri_mask] * c[valid_tri_mask]) / (4.0 * area[valid_tri_mask])
        ...
        if len(covered_vertices) < len(points_array):
            ...
            missing_indices = np.setdiff1d(
                all_indices, covered_vertices, assume_unique=True)
            ...
            if len(missing_indices) > 0:
                ...
                unique_v, unique_indices = np.unique(
                    sorted_vertices, return_index=True)
        ...
        if np.any(keep_indices):
            ...
            if len(s) > 0:
                ...
                adj: List[Set[int]] = [set() for _ in range(num_tri)]
                ...
                # 0-degree nodes (isolated) are dealt with later (keep if
                # originally kept)
                leaves = [i for i in range(
                    num_tri) if degrees[i] == 1 and not keep_indices[i]]
                ...
                while q_idx < len(queue):
                    ...
                    # But we must check if neighbor is not already eliminated
                    # (unlikely in tree traversal from leaves but safe)
                    for v in adj[u]:
                        if not eliminated[v]:
                            degrees[v] -= 1
        ...
        # kept_simplices = tri.simplices[keep_indices]
        # But we need indices relative to points_array, which tri.simplices
        # already provides.
        res = _extract_boundary_edges_fast(
            tri.simplices[keep_indices], points_array)

        if not res:
            # Fallback if fast extraction failed or produced nothing (e.g. no
            # cycles found)
            polys = [Polygon(t) for t in kept_triangles]
            concave_hull = unary_union(polys)
...
def _fallback_convex(points_array, cache_key):
    try:
        ...
        res = [[[float(p[0]), float(p[1])]
                for p in points_array[hull.vertices]]]
        _cache_result(cache_key, res)
        return res
    except Exception as e:
...
def _extract_boundary_edges_fast(triangles_indices, points_array):
    ...
    # Each triangle has 3 edges. We sort vertex indices per edge to handle
    # (u,v) == (v,u)
...
def _stitch_edges(edges, points_array):
    ...
    while edge_set:
        ...
        while curr != start:
            # Fallback to slower robust construction below
            break
    ...
    try:
        ...
        for comp in components:
            ...
            try:
                ...
                loops_coords.append(
                    [[float(points_array[i][0]), float(points_array[i][1])] for i in path])
            except Exception as e:
                print(f"Cycle finding failed: {e}")
                pass
    except Exception as e:
    ...
    for cluster in clusters:
        ...
        for p in cluster:
            if isinstance(p, (pd.Series, dict)):
                lat = p.get('latitude') if isinstance(
                    p, dict) else p['latitude']
                lon = p.get('longitude') if isinstance(
                    p, dict) else p['longitude']
    ...
    if len(clean_clusters) < 5:
        return [
            get_alpha_shape(
                c,
                alpha=alpha,
                auto_alpha_quantile=auto_alpha_quantile) if c else [] for c in clean_clusters]
