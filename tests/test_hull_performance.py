...
class HullPerformanceBenchmark:
    ...
    def generate_test_points(self, n_points: int, seed: int = 42) -> List[List[float]]:
    ...
    def generate_clustered_points(self, n_clusters: int, points_per_cluster: int, 
        ...
        for i in range(n_clusters):
            ...
            cluster_points = center + np.random.randn(points_per_cluster, 2) * 0.1
    ...
    def benchmark_single_hull(self, n_points: int, alpha: float = 1.0, 
        ...
        # Warm up cache
        _ = get_alpha_shape(points[:10], alpha=alpha, auto_alpha_quantile=auto_alpha_quantile)
        ...
        result = get_alpha_shape(points, alpha=alpha, auto_alpha_quantile=auto_alpha_quantile)
        end_time = time.time()
    ...
    def benchmark_batch_hulls(self, n_clusters: int, points_per_cluster: int, 
                              alpha: float = 1.0, auto_alpha_quantile: float = 0.95,
                              max_workers: int = None) -> Dict:
        """Benchmark batch hull computation."""
        logger.info(f"Benchmarking batch hulls: {n_clusters} clusters, {points_per_cluster} points each...")
        
        clusters = self.generate_clustered_points(n_clusters, points_per_cluster)
        
        # Warm up cache
        _ = compute_cluster_hulls(clusters[:2], alpha=alpha, auto_alpha_quantile=auto_alpha_quantile)
        ...
        # Measure time
        start_time = time.time()
        results = compute_cluster_hulls(clusters, alpha=alpha, auto_alpha_quantile=auto_alpha_quantile, 
    ...
    def benchmark_cache_effectiveness(self, n_points: int, iterations: int = 5) -> Dict:
        """Benchmark cache effectiveness by running same computation multiple times."""
        logger.info(f"Benchmarking cache effectiveness with {n_points} points, {iterations} iterations...")
        ...
        for i in range(iterations):
            start_time = time.time()
            result = get_alpha_shape(points, alpha=1.0, auto_alpha_quantile=0.95)
            end_time = time.time()
    ...
    def benchmark_scaling(self, point_counts: List[int], alpha: float = 1.0) -> List[Dict]:
        ...
        if len(results) > 1:
            ...
            logger.info(f"  Average scaling factor: {np.mean(scaling_factors):.2f}x")
        
        return results
    
    def benchmark_alpha_sensitivity(self, n_points: int, alpha_values: List[float]) -> List[Dict]:
        """Benchmark sensitivity to alpha parameter."""
        logger.info(f"Benchmarking alpha sensitivity with {n_points} points, alpha values: {alpha_values}")
    ...
    def run_comprehensive_benchmark(self) -> Dict:
        ...
        for n_clusters, points_per_cluster, max_workers in parallel_configs:
            result = self.benchmark_batch_hulls(n_clusters, points_per_cluster, max_workers=max_workers)
            all_results.append(result)
        ...
        for test_type, test_results in test_types.items():
            ...
            if test_type == 'single_hull':
                for r in test_results:
                    summary_lines.append(f"  {r['n_points']} points: {r['execution_time']:.4f}s")
            
            elif test_type == 'batch_hulls':
                for r in test_results:
                    summary_lines.append(f"  {r['n_clusters']} clusters ({r['total_points']} total points): {r['execution_time']:.4f}s")
            
            elif test_type == 'cache_effectiveness':
                for r in test_results:
                    summary_lines.append(f"  {r['n_points']} points, {r['iterations']} iterations: {r['speedup']:.2f}x speedup")
            
            elif test_type == 'alpha_sensitivity':
                for r in test_results:
                    summary_lines.append(f"  alpha={r['alpha']}: {r['execution_time']:.4f}s")
            
            elif test_type == 'parallel_execution':
                for r in test_results:
                    summary_lines.append(f"  {r['n_clusters']} clusters, {r['max_workers']} workers: {r['execution_time']:.4f}s")
