...
class TimeFilter:
    ...
    def detect_events(
        ...
        if not force_refresh:
            ...
            if cached_events:
                logger.info(
                    f"Loaded {len(cached_events)} detected events from DB cache.")
        ...
        high_activity_days = daily_counts[daily_counts >
        ...
        for i, days in enumerate(raw_events, 1):
            # Convert to int for JSON serializable
            total_entries = int(daily_counts.loc[days].sum())

            # Format dates
            start_date = datetime(dummy_year, 1, 1) + \
                pd.Timedelta(days=days[0] - 1)
            end_date = datetime(dummy_year, 1, 1) + \
                pd.Timedelta(days=days[-1] - 1)
    ...
    def filter_by_event(self, df: pd.DataFrame, event: Dict) -> pd.DataFrame:
        ...
        logger.info(
            f"Filtering for event: {event['label']} (Days {event['start_day']}-{event['end_day']})")
    ...
    def filter_by_events(
            ...
            df: pd.DataFrame,
            events: List[Dict],
            exclude: bool = True) -> pd.DataFrame:
        """Filter data by including ONLY or excluding the specified list of events."""
        if not events:
            return df
        ...
        labels = []
        for e in events:
            # e['days'] is a list of integers representing day of year
            target_days.update(e['days'])
            labels.append(e['label'])

        if exclude:
            logger.info(
                f"Excluding events: {', '.join(labels)} ({len(target_days)} days)")
            # Keep rows where dayofyear is NOT in target_days
            return df[~df['date'].dt.dayofyear.isin(target_days)]
        else:
            logger.info(
                f"Including only events: {', '.join(labels)} ({len(target_days)} days)")
            # Keep rows where dayofyear IS in target_days
            return df[df['date'].dt.dayofyear.isin(target_days)]

    def exclude_events(
            self,
            df: pd.DataFrame,
            events_to_exclude: List[Dict]) -> pd.DataFrame:
    ...
    def filter_by_time_range(
        ...
        return df[(df['date'].dt.year >= start_year)
