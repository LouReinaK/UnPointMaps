...
def compute_cluster_hulls(clusters, alpha: float | None = 1.0, auto_alpha_quantile=0.95, max_workers=None):
    ...
    # Preprocess inputs to clean lists of coordinates
    ...
    # Sequential for small number of clusters to avoid overhead
    ...
    if max_workers is None:
        # Limit workers to avoid freezing the machine given this might run concurrently with other heavy tasks
        # Or just use logical cpu count
        max_workers = min(multiprocessing.cpu_count(), 8)

    # Use ProcessPoolExecutor for CPU-bound hull computation
    # Prepare arguments
    tasks = [(c, alpha, auto_alpha_quantile) for c in clean_clusters]
    
    results = []
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        # We process skipping empty clusters
        # But we need to maintain order.
        # tasks contains potentially empty lists? get_alpha_shape handles simple cases fast.
        # Check get_alpha_shape implementation: it handles small inputs.
        
        # map returns consistent order
