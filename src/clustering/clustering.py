...
def plot_k_distance(data, k):
    ...
    if len(points) < k:
        print(f"Not enough points ({len(points)}) to compute {k}-nearest neighbors.")
        return
...
def plot_clusters(data, titles=None):
    ...
    if data and isinstance(data, list) and len(data) > 0:
        ...
        if isinstance(first_item, list) and len(first_item) > 0:
            ...
            if isinstance(first_sub, (list, np.ndarray)) and len(first_sub) > 0:
                # Si le contenu est numérique, c'est un point => data est une liste de clusters (Single)
                # Sinon (si c'est un itérable), c'est un cluster => data est une liste de résultats (Multiple)
                try:
    ...
    # squeeze=False assure que axes est toujours un tableau 2D, flatten() le rend 1D
            ...
            if len(points) > 0:
                ...
                if len(points) >= 3:
                    ...
                    if len(unique_points) >= 3:
                        try:
                            # QJ: Joggle inputs. This is robust against coplanar/collinear points
                            hull = ConvexHull(unique_points, qhull_options='QJ')
                            hull_indices = np.append(hull.vertices, hull.vertices[0])
                            ax.plot(unique_points[hull_indices, 1], unique_points[hull_indices, 0], '--', alpha=0.5)
                        except Exception as e:
                            # Silently fail or log sparingly to avoid console spam
                            # print(f"Could not compute hull for cluster {i} in plot {idx}: {e}")
                            pass
        
        title = titles[idx] if titles else (f'Visualisation des Clusters ({idx+1})' if num_plots > 1 else 'Visualisation des Clusters')
        ax.set_title(title)
